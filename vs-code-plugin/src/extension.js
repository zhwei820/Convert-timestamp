// SOL 价格状态栏：把 Solana 最新价常驻在 VS Code 状态栏上，编辑器开着就一直能看到
// 相当于 Chrome 扩展里的图标 badge（见 ../../src/sol-price-badge.js），但这边的宿主进程不会被回收，
// 所以定时刷新直接用 setInterval 就行，不需要 chrome.alarms 那套唤醒机制，刷新频率也不再被 1 分钟下限卡住
...
// 涨绿跌红（加密行情通用配色）；想换成 A 股的红涨绿跌，把 solPrice.invertColors 打开即可
...
let ctx;
/** 最近一次成功拿到的行情，刷新失败时灰着继续显示 */
let lastSnapshot = null;
/** 正在请求中，避免手动刷新和定时刷新叠在一起打两次 */
let inFlight = false;
let consecutiveFailures = 0;
/** 状态栏当前显示的是不是过期数据（决定要不要在 window 重新聚焦时补刷） */
...
function createStatusBarItem() {
    ...
    statusBarItem = vscode.window.createStatusBarItem(cfg.alignment, cfg.priority);
    ...
    statusBarItem.show();
    // 故意不 push 进 ctx.subscriptions：改 alignment / priority 会反复重建 item，
    // 那样 subscriptions 会越堆越长。这里的 item 统一由 deactivate 收尾
}
...
async function refresh(options) {
    ...
    try {
        ...
        log("刷新失败: " + message);
        // 拉不到就把上一次的价格灰掉继续显示，别让状态栏空着——空着比旧价格更容易让人误判
...
function onConfigChanged(event) {
    ...
    // alignment / priority 是 createStatusBarItem 的入参，改了只能重建 item
    if (event.affectsConfiguration("solPrice.alignment") || event.affectsConfiguration("solPrice.priority")) {
        createStatusBarItem();
    }
